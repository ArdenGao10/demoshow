...
(function () {
  ...
  // 记下自己被加载时的真实 src,导出嵌入代码时原样引用(保证版本一致)
  const SCRIPT_SRC = (() => {
    try {
      const me = document.currentScript
        || Array.from(document.scripts).reverse().find(s => /demi-widget(@[\w.-]+)?(\.\w+)?\.js/.test(s.src || ""));
      return me && me.src ? me.src : "";
    } catch (e) { return ""; }
  })();
  ...
  function exportEditSnippet() {
    ...
    const widgetUrl = SCRIPT_SRC || (location.origin || "") + `/demi-widget@${DEMI_WIDGET_VERSION}.js`;
